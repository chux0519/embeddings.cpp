...
#include "ggml.h"
#include "gguf.h"
#include "bert.h"    // 引入你的 BackendContext 等定义
#include "gte.h"     // 引入你的 GteBertConfig 等定义
#include "jina_bert.h" // 引入你的 JinaBertConfig 等定义

// 辅助函数：从字符串获取 ggml_type
...
// 检查张量是否应该被量化 (可根据你的模型调整)
static bool should_quantize_tensor(const std::string& name, const ggml_tensor *tensor) {
...
// 写入 GTE 模型的元数据
static void write_gte_hparams(gguf_context * ctx, const embeddings::GteBertConfig& hparams) {
    gguf_set_val_u32(ctx, "vocab_size", hparams.vocab_size);
    // 注意你的Python脚本用的是 max_position_embedding
    gguf_set_val_u32(ctx, "max_position_embedding", 8192); // Snowflake-arctic-embed-m is 8192
