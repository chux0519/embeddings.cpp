...
#include "ggml.h"
#include "gguf.h"
#include "utils.h" // 你的 get_u32, get_f32 等函数
#include "gguf_utils.h" // 我们之前创建的头文件

// 从 llama.cpp 借用的辅助函数，用于将 kv 对从一个 gguf 上下文复制到另一个
void gguf_kv_set_from_ctx(gguf_context * ctx_out, gguf_context * ctx_in, int i) {
    ...
    switch (type) {
        case GGUF_TYPE_UINT8:   gguf_set_val_u8  (ctx_out, key, gguf_get_val_u8  (ctx_in, i)); break;
        case GGUF_TYPE_INT8:    gguf_set_val_i8  (ctx_out, key, gguf_get_val_i8  (ctx_in, i)); break;
        case GGUF_TYPE_UINT16:  gguf_set_val_u16 (ctx_out, key, gguf_get_val_u16 (ctx_in, i)); break;
        case GGUF_TYPE_INT16:   gguf_set_val_i16 (ctx_out, key, gguf_get_val_i16 (ctx_in, i)); break;
        case GGUF_TYPE_UINT32:  gguf_set_val_u32 (ctx_out, key, gguf_get_val_u32 (ctx_in, i)); break;
        case GGUF_TYPE_INT32:   gguf_set_val_i32 (ctx_out, key, gguf_get_val_i32 (ctx_in, i)); break;
        case GGUF_TYPE_FLOAT32: gguf_set_val_f32 (ctx_out, key, gguf_get_val_f32 (ctx_in, i)); break;
        case GGUF_TYPE_BOOL:    gguf_set_val_bool(ctx_out, key, gguf_get_val_bool(ctx_in, i)); break;
        case GGUF_TYPE_STRING:  gguf_set_val_str (ctx_out, key, gguf_get_val_str (ctx_in, i)); break;
        ...
        case GGUF_TYPE_UINT64:  gguf_set_val_u64 (ctx_out, key, gguf_get_val_u64 (ctx_in, i)); break;
        case GGUF_TYPE_INT64:   gguf_set_val_i64 (ctx_out, key, gguf_get_val_i64 (ctx_in, i)); break;
        case GGUF_TYPE_FLOAT64: gguf_set_val_f64 (ctx_out, key, gguf_get_val_f64 (ctx_in, i)); break;
        default: break;
    }
}
...
int main(int argc, char **argv) {
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <input_model.gguf> <output_model.gguf> <qtype>\n", argv[0]);
    ...
    // 1. 打开输入的 GGUF 文件并加载元数据
    struct ggml_context * ctx_meta = nullptr; // 这个 context 只用于保存张量元数据
    ...
    for (int i = 0; i < n_tensors; ++i) {
        const char * name = gguf_get_tensor_name(ctx_in, i);
        struct ggml_tensor * tensor = ggml_get_tensor(ctx_meta, name);
        enum ggml_type new_type = should_quantize_tensor(name, tensor) ? out_type : tensor->type;
        size_t nbytes_tensor = ggml_nbytes(tensor);
        std::vector<char> data(nbytes_tensor);
        fseek(f_in, gguf_get_data_offset(ctx_in) + gguf_get_tensor_offset(ctx_in, i), SEEK_SET);
        fread(data.data(), 1, data.size(), f_in);
        void * new_data = nullptr;
        size_t new_size = nbytes_tensor;
        printf("[%4d/%4d] %-40s | type: %-6s -> %-6s ... ",
            i + 1, n_tensors, name, ggml_type_name(tensor->type), ggml_type_name(new_type));
        fflush(stdout);
        if (should_quantize_tensor(name, tensor)) {
            const size_t nelements = ggml_nelements(tensor);
            work_f32.resize(nelements);
            if (tensor->type == GGML_TYPE_F16) {
                ggml_fp16_to_fp32_row((ggml_fp16_t*)data.data(), work_f32.data(), nelements);
            } else {
            ...
            int64_t rows_per_chunk = std::max<int64_t>(1, (min_chunk + row_size - 1) / row_size);
            ...
            for (int64_t chunk_idx = 0; chunk_idx < nchunk; ++chunk_idx) {
                ...
                q_size += ggml_quantize_chunk(
                    new_type,
                    work_f32.data() + offset,
                    (uint8_t *)work_q.data() + offset * qtype_size / qk,
                    0,
                    n_chunk_rows,
                    row_size,
                    nullptr
            ...
            GGML_ASSERT(gguf_get_tensor_size(ctx_out, gguf_find_tensor(ctx_out, name)) == q_size);
