...
extern "C" {
    ...
    // Snowflake/GTE helper: split Q/K/V from packed QKV, apply NeoX RoPE
    // to Q/K from precomputed cos/sin caches, and write the flash-attention layout.
    GGML_API struct ggml_tensor * ggml_gte_qkv_rope(
            struct ggml_context * ctx,
    ...
    // Snowflake/GTE helper: gather one CLS token column per batch row.
    // Positions < 0 produce a zero vector for fully masked rows.
    GGML_API struct ggml_tensor * ggml_gte_cls_pool(
            struct ggml_context * ctx,
            struct ggml_tensor  * inp,
            struct ggml_tensor  * positions);

    // Snowflake/GTE helper: split [up, gate], then compute up * GELU(gate).
    GGML_API struct ggml_tensor * ggml_gte_geglu(
            struct ggml_context * ctx,
            struct ggml_tensor  * up_gate,
            int                   hidden_features);

    // Snowflake/GTE helper: layer norm followed by per-channel weight and bias.
    ...
    GGML_API struct ggml_tensor * ggml_gte_linear(
            struct ggml_context * ctx,
            struct ggml_tensor  * weight,
            struct ggml_tensor  * inp,
            struct ggml_tensor  * bias);
